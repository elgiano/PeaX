...
double PeaXShift::getTrueFreq(int bin_i, float phaseDiff){
...
double PeaXShift::getPhaseCorrection(int bin_i, double freq){
...
void PeaXShift::init(SndBuf* buf, int numbins){
  m_numbins = numbins;
  ...
  data_a = new PeaXShift_Data(mWorld, numbins);
  data_b = new PeaXShift_Data(mWorld, numbins);
  ...
  memset(m_prevPhases, 0, numbins * sizeof(float));
    ...
    Print("PeaXShift complains: block size not 64, you have %d\n", mWorld->mFullRate.mBufLength);
}
...
void PeaXShift::getFreqs(SCPolarBuf* fft, float* prevPhases){
	float currPhase;
...
void PeaXShift::findPeaks(PeaXShift_Data* data, SCPolarBuf* fft){

	//swap new peaks to old; current now safe to overwrite;
	PeaXShift_peak *peaks = data->peaks;
	int numpeaks = 0;
...
void PeaXShift::transpose(PeaXShift_Data* data, SCPolarBuf* a, SCPolarBuf* dstBuf){
...
void PeaXShift::combinePeaks(PeaXShift_Data* a, PeaXShift_Data* b, SCPolarBuf* dstBuf){
	...
	PeaXShift_peak *peaks_a = a->peaks;
	PeaXShift_peak *peaks_b = b->peaks;
